...
nasa_api_key = "OOIEOudOB4dacaPgthiodkRIjfLddzDOgp0hYavQ"
...
if r.status_code == 200:
	...
	ast_safe = []
	ast_hazardous = []
#total info about all asteroids today
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count'])
		print("Asteroid count today: " + str(ast_count))
#checks aseroid diameter and checks if it is dangarous
		if ast_count > 0:
...
#checks asteroid approch data
					if len(val['close_approach_data']) > 0:
						...
						tmp_ast_speed = -1
						tmp_ast_miss_dist = -1
#collects and prints all info what was worken before
					print("------------------------------------------------------- >>")
...
#if there is error in data or no info 
else:
